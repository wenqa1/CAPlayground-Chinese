...
export function StatesPanel() {
  ...
  const stateNameMap: Record<string, string> = {
    'Base State': t("baseState"),
    'Locked': t("locked"),
    'Unlock': t("unlock"),
    'Sleep': t("sleep"),
    'Locked Light': t("lockedLight"),
    'Unlock Light': t("unlockLight"),
    'Sleep Light': t("sleepLight"),
    'Locked Dark': t("lockedDark"),
    'Unlock Dark': t("unlockDark"),
    'Sleep Dark': t("sleepDark"),
  };

  const displayStateName = (s: string) => stateNameMap[s] || s;
  ...
  const findLayerName = (layerId: string): string => {
    const findInLayers = (layers: AnyLayer[]): string | null => {
      for (const layer of layers) {
        if (layer.id === layerId) return layer.name || t("unnamedLayer");
    ...
    return findInLayers(current?.layers || []) || t("unknownLayer");
  };
    ...
    <Card className="p-0 gap-0 h-full flex flex-col" data-tour-id="states-panel">
      <div className="flex items-center justify-between px-3 py-2 border-b shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Dialog open={viewAllOpen} onOpenChange={setViewAllOpen}>
            ...
            <DialogHeader>
              <DialogTitle>{t("transitionsOverview")}</DialogTitle>
            </DialogHeader>
            <div className="flex-1 overflow-y-auto space-y-6 pr-2">
              {/* States Summary */}
              <div>
                <h3 className="text-sm font-semibold mb-2">{t("availableStates")}</h3>
              ...
              {Object.keys(overridesByLayer).length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold mb-3">{t("stateOverridesByLayer")}</h3>
              ...
              {Object.keys(overridesByLayer).length === 0 && (
                <div className="text-center text-muted-foreground py-8">
                  ...
                  <p className="text-xs mt-1">{t("transitionHint")}</p>
                </div>
              )}
            </div>
            </DialogContent>
          ...
          {!gyroEnabled && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64 p-2">
                <DropdownMenuLabel className="text-xs">{t("stateSettings")}</DropdownMenuLabel>
                <div className="space-y-1">
                  <div className="flex items-center justify-between py-1.5 px-1">
                    <span className="text-xs text-muted-foreground">{t("appearanceSplit")}</span>
                    <Switch checked={split} onCheckedChange={onToggleSplit} />
                  </div>
                  <p className="text-[10px] text-muted-foreground px-1 pb-1">
                    {t("appearanceSplitWarning")}
                  </p>
                </div>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      <div className="flex-1 overflow-hidden p-3">
        <div className="text-sm rounded-lg border bg-card shadow-sm divide-y flex flex-col overflow-hidden">
          <div className="px-2 py-2 font-medium select-none bg-muted/30">{t("projectStates")}</div>
          <div className="flex-1 overflow-auto">
            ...
            >
              <div className="truncate flex-1">{t("baseState")}</div>
            </button>
            {states.map((s) => (
              <button
                key={s}
              ...
              >
                <div className="truncate flex-1">{displayStateName(s)}</div>
              </button>
            ))}
          </div>
        </div>
