      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <Card className="border-border/80 shadow-none w-full max-w-md">
          ...
          <CardContent className="flex items-center gap-2 text-muted-foreground">
            <Loader2 className="h-4 w-4 animate-spin" /> {t("verifyingSession")}
          </CardContent>
        </Card>
      </div>
    )
      ...
      <Card className="border-border/80 shadow-none w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className={status === "ready" ? "text-2xl md:text-3xl" : undefined}>
            {status === "need_username" ? t("chooseUsername") : status === "ready" ? t("signedIn") : t("signInFailed")}
          </CardTitle>
        </CardHeader>
          ...
          {status === "need_username" ? (
            <div className="space-y-4">
              <div>
                ...
                <Input id="username" value={username} onChange={(e) => setUsername(e.target.value)} placeholder={t("usernamePlaceholder")} className="mt-1" />
              ...
              <div className="flex gap-2 justify-center">
                ...
                >
                  {saving ? tc("saving") : t("saveUsername")}
                </Button>
              </div>
            </div>
          ) : status === "ready" ? (
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2">
                <Button className="w-full" onClick={() => (window.location.href = "/")}>{tc("continue")}</Button>
                <Button className="w-full" variant="outline" onClick={() => (window.location.href = "/projects")}>{t("createProject")}</Button>
                <Button className="w-full" variant="outline" onClick={() => (window.location.href = "/dashboard")}>{t("accountDashboard")}</Button>
                <Button
                  className="w-full"
                  variant="destructive"
                  onClick={async () => {
            ...
            <div className="text-center space-y-4">
              ...
              <Button variant="outline" onClick={() => (window.location.href = "/signin")}>{t("backToSignIn")}</Button>
            </div>
          )}
        </CardContent>
      </Card>
